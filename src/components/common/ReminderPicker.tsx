  ...
  const dateShortcuts = useMemo(() => {
    ...
    return [
      { icon: Sun, color: '#22C55E', label: 'Hoy', hint: DAY_SHORT[getDay(today)], date: today },
      {
        ...
        color: '#F59E0B',
        label: 'Mañana',
        hint: DAY_SHORT[getDay(tomorrow)],
        date: tomorrow,
      },
      {
        icon: Sofa,
  ...
  const panelNode =
    inline || open ? (
      ...
      >
        {/* Header — no scrollea */}
        <div
          className="shrink-0 border-b px-4 pt-3 pb-0"
          ...
          <div className="flex">
            <button
              ...
              style={{
                ...
                color: activeTab === 'datetime' ? 'var(--text-primary)' : 'var(--text-muted)',
                borderBottom:
                  activeTab === 'datetime'
            ...
            <div>
              ...
              <div className="mb-2 flex gap-1">
                {dateShortcuts.slice(0, 3).map(({ icon: Icon, color, label, hint, date }) => (
                  <button
                    ...
                    onMouseLeave={(e) =>
                      (e.currentTarget.style.backgroundColor = isSameDay(selectedDate, date)
                        ? color + '22'
                        : 'var(--bg-secondary)')
                    }
                  >
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
                  {format(selectedDate, "d 'de' MMM", { locale: es })}, {selectedHour}:
                  {selectedMinute}
                </span>
                <button
