...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'exchange') {
      ...
      const { error: upsertError } = await adminClient.from('calendar_integrations').upsert(
        {
          user_id: user.id,
          provider: 'google',
    ...
    const status =
      message.includes('Invalid or expired JWT') || message.includes('Missing or invalid')
        ? 401
        : 500
