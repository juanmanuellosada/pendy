...
### Requirement: Cobertura unitaria del hook

El sistema SHALL incluir tests unitarios en `src/hooks/useInstallPrompt.test.ts` que cubran los siguientes escenarios mockeando `window.matchMedia`, `navigator.platform`/`navigator.userAgentData`, `window.addEventListener('beforeinstallprompt', ...)` y `isTauri()`:

1. Browser Chromium desktop con PWA no instalada → `canInstallPwa === true`.
2. PWA standalone (display-mode standalone) → `isStandalone === true`, `canInstallPwa === false`.
3. Tauri (window.**TAURI_INTERNALS** presente) → `isTauri === true`.
4. Firefox simulado (sin beforeinstallprompt) → `canInstallPwa === false`.
5. iOS simulado → `os === 'ios'`.
6. Linux detection → `os === 'linux'`.
7. Llamada a `promptInstallPwa()` sin evento diferido → `'unavailable'`.

#### Scenario: Run de la suite

- **WHEN** se ejecuta `pnpm test`
- **THEN** todos los tests del hook pasan junto con la suite existente
