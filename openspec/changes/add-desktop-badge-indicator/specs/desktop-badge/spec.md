...
## ADDED Requirements (PWA fallback)

### Requirement: Preservación de la ruta PWA

Cuando `isTauri()` es falso, el sistema SHALL mantener el comportamiento previo a este cambio sin diferencias observables:

- Si `'setAppBadge' in navigator`, llamar `navigator.setAppBadge(count)` o `navigator.clearAppBadge()` desde el main thread.
- Adicionalmente, enviar `postMessage({ type: 'SET_BADGE', count })` al Service Worker para soporte iOS PWA.
- El Service Worker (`public/sw.js`) continúa procesando `SET_BADGE` como hasta ahora.

#### Scenario: PWA en Chromium con Badging API

- **WHEN** el bundle corre como PWA en Chrome desktop con count = 3
- **THEN** se llama `navigator.setAppBadge(3)` y se envía `{ type: 'SET_BADGE', count: 3 }` al SW

#### Scenario: PWA en navegador sin Badging API

- **WHEN** el bundle corre en un navegador sin `setAppBadge`
- **THEN** el hook no lanza errores y simplemente no publica badge (degradación silenciosa, igual que hoy)

### Requirement: Sin regresión de bundle web

El paquete `@tauri-apps/api` MUST aparecer como `optionalDependencies` o cargarse vía dynamic import gateado por `isTauri()`, de forma que el bundle PWA producido por `pnpm build` no incluya su código.

#### Scenario: Inspección de bundle PWA

- **WHEN** se inspecciona el output de `pnpm build`
- **THEN** ningún chunk del bundle contiene strings de `@tauri-apps/api` ni símbolos de `__TAURI_INVOKE__` fuera del check gateado
